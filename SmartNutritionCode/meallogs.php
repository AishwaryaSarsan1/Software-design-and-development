...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && isset($_POST['meal_type'])) {
    ...
    if ($foodId) {
        // ONLY use non-expired pantry items for logging
        $st = $pdo->prepare("
    ...
    /* Decrement pantry quantity if a pantry item was logged */
    if ($foodId) {
        $upd = $pdo->prepare("
            UPDATE food_items
...
$logs = $pdo->prepare("
  SELECT ml.*,
         COALESCE(ml.recipe_title, ml.meal_name, fi.name) AS item_name
  FROM meal_logs ml
  LEFT JOIN food_items fi
    ON fi.id = ml.food_item_id
   AND fi.user_id = ml.user_id
  WHERE ml.user_id = ?
...
/* Pantry dropdown: group duplicate names, NON-EXPIRED ONLY */
$fiStmt = $pdo->prepare("
  SELECT
    MIN(id)       AS id,       -- representative row id
    MIN(name)     AS name,     -- keep a nice-looking name
        ...
        <div>
          ...
          <select name="food_item_id">
            ...
            <?php foreach ($pantry as $f):
              $qty  = isset($f['quantity']) ? (float)$f['quantity'] : 0.0;
              $unit = isset($f['unit']) ? (string)$f['unit'] : '';
              $qtyText = '';
              if ($qty > 0) {
