...
$st = $pdo->prepare('SELECT name, cal_goal FROM users WHERE id = ? LIMIT 1');
$st->execute([$userId]);
$userRow  = $st->fetch();

$userName = $userRow['name'] ?? ($_SESSION['user_name'] ?? 'User');
$_SESSION['user_name'] = $userName;

$calGoal = max(1, (int)($userRow['cal_goal'] ?? 0));  // <- corrected line

/* -------- Dashboard data -------- */
# Pantry counts
// Non-expired items only for the "Pantry Items" card and "Fresh" count
...
$pantryCount = (int)$pantryStmt->fetchColumn();
...
$wasteCount = (int)$wasteStmt->fetchColumn();

// Fresh = non-expired items
$freshCount = $pantryCount;

# Calories today – ALWAYS based on consumed_at (timestamp)
$st = $pdo->prepare("
...
# Recent meals (show latest 8)
$q = $pdo->prepare("
  SELECT ml.*, COALESCE(ml.recipe_title, ml.meal_name, fi.name) AS item_name
  FROM meal_logs ml
  ...
  <div style="display:flex;align-items:center;gap:.6rem">
    ...
    <div class="profile">
      ...
      <div id="profileMenu" class="dropdown" role="menu" aria-hidden="true">
        ...
        <div class="item">
          <a href="?logout=1" style="text-decoration:none;color:inherit"><span class="t">Sign Out</span></a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <div class="muted" style="margin-top:.3rem">
        of <?= $calGoal ?> kcal goal (<?= pct($calToday,$calGoal) ?>%)
      </div>
    </div>
  ...
  <div class="grid" style="margin-top:1.5rem">
      ...
      <?php else: ?>
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Date</th><th>Meal</th><th>Food / Recipe</th><th>Qty</th><th>Calories</th>
              </tr>
            </thead>
            <tbody>
    ...
    <div class="card">
      ...
      <div class="muted" style="margin-top:.4rem">
        Fresh: <strong><?= $freshCount ?></strong> ·
        Expired: <strong><?= $wasteCount ?></strong>
      </div>
    </div>
  </div>
</div>
...
const fresh = <?= (int)$freshCount ?>;
const expired = <?= (int)$wasteCount ?>;
