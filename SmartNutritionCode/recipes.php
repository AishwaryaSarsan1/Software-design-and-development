...
/* ===== Load user's pantry (non-expired, aggregate by name) ===== */
...
/* Only non-expired items; sort by nearest expiry; then aggregate by name */
$stmt = $pdo->prepare("
  SELECT id, name, quantity, unit, expiry_date, created_at
  FROM food_items
  WHERE user_id = ?
...
/* Aggregate per name (case-insensitive) to get total quantity */
$agg = [];
foreach ($rawItems as $row) {
  ...
  if (!isset($agg[$key])) {
    ...
    $currentExpiry = $agg[$key]['expiry_date'];
    $newExpiry     = $row['expiry_date'] ?? null;
    if ($currentExpiry === null || $currentExpiry === '' ||
        ($newExpiry !== null && $newExpiry !== '' && $newExpiry < $currentExpiry)) {
      $agg[$key]['expiry_date'] = $newExpiry;
    }
  }
}
$items = array_values($agg);

/* Sort aggregated items by earliest expiry (nulls last) */
usort($items, function($a, $b) {
  $ea = $a['expiry_date'] ?? null;
  $eb = $b['expiry_date'] ?? null;
  if ($ea === $eb) return 0;
  if ($ea === null || $ea === '') return 1;
  if ($eb === null || $eb === '') return -1;
  return strcmp($ea, $eb);
});
      ...
      <form class="find" method="get" action="recipes.php" novalidate>
        ...
        <div class="choices">
          ...
          <?php else: foreach ($items as $row):
            $name    = $row['name'];
            $checked = in_array($name, $rawSelected, true) ? 'checked' : '';
            $qty     = isset($row['quantity']) ? (float)$row['quantity'] : 0.0;
            $unit    = isset($row['unit']) ? (string)$row['unit'] : '';
            $qtyText = '';
            if ($qty > 0) {
              $prettyQty = rtrim(rtrim(number_format($qty, 2), '0'), '.');
              $qtyText   = ' ('.$prettyQty.($unit !== '' ? ' '.$unit : '').')';
            }
          ?>
            <label class="chip">
              <input type="checkbox" name="ing[]" value="<?= e($name) ?>" <?= $checked ?>>
              <?= e($name . $qtyText) ?>
            </label>
        ...
        <p class="small" style="margin-top:.5rem;">
          We automatically ignore expired items and duplicate ingredients. Counts reflect your current pantry.
        </p>
      </form>
