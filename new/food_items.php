...
function ensureColumn(PDO $pdo, string $col, string $definition) {
  $stmt = $pdo->prepare("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'food_items' AND COLUMN_NAME = ?");
...
function fetchNutritionFromApiNinjas(string $query): array {
  ...
  @file_put_contents(__DIR__ . '/api_ninjas_last.json', date('c') . " HTTP:$code\n" . ($err ?: $res) . "\n\n", FILE_APPEND);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name']) && !isset($_POST['update'])) {
  ...
  $storage = in_array($_POST['storage_type'] ?? 'pantry', ['pantry','refrigerator','freezer']) ? $_POST['storage_type'] : 'pantry';
